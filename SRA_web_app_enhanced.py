...
def cleanup_ollama_processes():
    """Clean up any orphaned Ollama model processes and SRA script processes, but preserve the main Ollama service."""
    try:
        # Method 1: Clean up specific model runner processes (not the main service)
        ...
        # Method 3: DO NOT kill main Ollama service (port 11434) - this was the bug!
        # The main service needs to stay running for model installation and listing
        # Only clean up if there are zombie processes that aren't the main service
...
def cleanup_analysis_processes_only():
    """Clean up only SRA analysis processes, preserving Ollama service completely."""
    try:
        # Clean up SRA script processes (but not current process)
        current_pid = os.getpid()
        sra_result = subprocess.run(['pgrep', '-f', 'SRA_fetch_1LLM_improved.py'], 
                                  capture_output=True, text=True)
        if sra_result.returncode == 0:
        ...
        st.error(f"Error during analysis cleanup: {e}")
        return False

def ensure_ollama_service_running():
    """Ensure Ollama service is running after analysis completion."""
    try:
        ollama_cmd = find_ollama_binary()
        
        # Check if Ollama service is responding
        result = subprocess.run([ollama_cmd, "list"], capture_output=True, text=True, timeout=10)
        if result.returncode != 0:
            ...
            try:
                ...
                # Check again
                result2 = subprocess.run([ollama_cmd, "list"], capture_output=True, text=True, timeout=10)
                if result2.returncode == 0:
                    return True, "✅ Ollama service restarted successfully"
                else:
                    return False, "❌ Failed to restart Ollama service"
            except Exception as e:
                return False, f"❌ Failed to start Ollama service: {e}"
        else:
            return True, "✅ Ollama service is running"
    except Exception as e:
        return False, f"❌ Error checking Ollama service: {e}"

def find_ollama_binary():
    """Find the ollama binary in common locations."""
    common_paths = [
    ...
    # Finally try just 'ollama' (relies on PATH)
    return 'ollama'
...
def list_ollama_models():
    ...
    try:
        ...
        st.error("Ollama not found. Please install Ollama or check your PATH.")
    except subprocess.TimeoutExpired:
        st.error("Timeout while checking Ollama models.")
    except Exception as e:
...
def pull_ollama_model(model_name: str):
    ...
    try:
        ...
        try:
            test_result = subprocess.run([ollama_cmd, "list"], capture_output=True, text=True, timeout=10)
            if test_result.returncode != 0:
                return f"❌ Ollama service is not running. Please start it with: {ollama_cmd} serve"
        except:
            return f"❌ Cannot connect to Ollama service. Please ensure it's running."
        
        # Now try to pull the model
        result = subprocess.run([ollama_cmd, "pull", model_name], capture_output=True, text=True, timeout=600)
        if result.returncode == 0:
            return f"✅ Model '{model_name}' installed successfully."
        else:
            error_msg = result.stderr if result.stderr else result.stdout
            return f"❌ Failed to install '{model_name}': {error_msg}"
    except FileNotFoundError:
        return "❌ Ollama command not found – please install Ollama first or check your PATH."
    except subprocess.TimeoutExpired:
        return f"❌ Timeout installing '{model_name}' - try again later (large models can take several minutes)."
    except Exception as e:
        return f"❌ Error installing '{model_name}': {str(e)}"
...
with tab1:
    ...
    with col1:
        ...
        if installed_models:
            ...
            elif st.session_state.get('selected_model') in model_options:
                default_index = model_options.index(st.session_state.get('selected_model'))
            
            selected_model = st.selectbox("Choose AI Model", model_options, index=default_index)
            
            # Check if model changed and cleanup if needed
            if st.session_state.get('selected_model') != selected_model:
                if st.session_state.get('selected_model') is not None and st.session_state.analysis_running:
            ...
            if selected_model in installed_models:
                ...
                st.warning("⚠️ Model not installed")
        else:
    ...
    with col3:
        available_to_install = [m for m in POPULAR_MODELS if m not in installed_models] if installed_models else POPULAR_MODELS
        install_model = st.selectbox("Install model:", [""] + available_to_install, key="main_install_selector")
        if st.button("📥 Install") and install_model:
            with st.spinner(f"Installing {install_model}..."):
                ...
                if "✅" in result:
                    ...
                    st.session_state.selected_model = install_model
                else:
                    st.error(result)
            st.rerun()

    with col4:
        if st.button("🧹 Cleanup", help="Stop running model processes (keeps main service)"):
            with st.spinner("Cleaning up model processes..."):
            ...
            while True:
                try:
                    ...
                    if msg_type == 'log':
                        ...
                        # Clean up processes and temp files (preserve Ollama service)
                        cleanup_analysis_processes_only()
                        st.session_state.current_process = None
                        try:
                            os.unlink(st.session_state.keywords_file)
                        except:
                        ...
                        # Ensure Ollama service is still running for future use
                        service_ok, service_msg = ensure_ollama_service_running()
                        st.session_state.analysis_logs.append(f"🧹 Analysis cleanup completed")
                        st.session_state.analysis_logs.append(service_msg)
                        break
                    elif msg_type == 'error':
                        st.session_state.analysis_running = False
                        st.session_state.analysis_logs.append(f"❌ Error: {content}")
                        
                        # Clean up processes and temp files on error (preserve Ollama service)
                        cleanup_analysis_processes_only()
                        st.session_state.current_process = None
                        try:
                            os.unlink(st.session_state.keywords_file)
                        ...
                        # Ensure Ollama service is still running for future use
                        service_ok, service_msg = ensure_ollama_service_running()
                        st.session_state.analysis_logs.append("🧹 Analysis cleanup completed after error")
                        st.session_state.analysis_logs.append(service_msg)
                        break
...
if installed_models:
    st.sidebar.success(f"✅ {len(installed_models)} AI models available")
    if current_model and current_model in installed_models:
        st.sidebar.info(f"🎯 Current: {current_model}")
    else:
        st.sidebar.warning("⚠️ No model selected")
else:
    st.sidebar.error("❌ No AI models found")

# Analysis script status
if os.path.exists("SRA_fetch_1LLM_improved.py"):
    st.sidebar.success("✅ Analysis script ready")
else:
    st.sidebar.error("❌ Analysis script missing")
...
with st.sidebar.expander("ℹ️ Help & Tips"):
    st.markdown("""
    **Analysis Tab:**
    - Select AI model and enter keywords
    - Live progress tracking with metrics
    - Auto-generates visualizations
    - All model management in main interface
